...
public class TapWatchXtrasButtonAndBackAutoplayOffTest extends FalseBackgroundVideoServicesBaseTest {

    @Factory(dataProvider = Config.StaticProps.DEFAULT_DATA_PROVIDER, dataProviderClass = DataProviderManager.class)
    public TapWatchXtrasButtonAndBackAutoplayOffTest(String runParams) {
    ...
    public void tapWatchXtrasButtonAndBackAutoplayOffAndroidTest() {
        ...
        if (backgroundServiceVideoNotEnabled) {
            ...
            clips.clipTitle(clipTitle).waitForVisible().pause(Config.StaticProps.LARGE_PAUSE);
            videoPlayer.verifyVideoIsNotPlayingByScreenshots(5);
            videoPlayer.largePlayBtn().waitForVisible().tap().waitForNotPresent();
